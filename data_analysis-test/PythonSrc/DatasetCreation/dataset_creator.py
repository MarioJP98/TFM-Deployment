...
class my_trackset():
    ...
    def __init__(self):
        array_length = 10
        self.ar = multiprocessing.Array(
            'l', array_length)  # l for long, i not enough
...
def release_lock_track(trackid):
    ...
    if not trackid in TRACKSET:
        TRACKSET_LOCK.release()
        print 'WARNING: releasing a song you dont own, trackid=', trackid
        sys.stdout.flush()
        return False
...
def create_track_file_from_trackid(maindir, trackid, song, artist, mbconnect=None):
    ...
    res = create_track_file(maindir, trackid, track,
                            song, artist, mbconnect=mbconnect)
    return res
...
def create_track_file_from_song(maindir, song, artist, mbconnect=None):
    ...
    while True:
        try:
            ...
            # should not happen according to EN guys, but still does...
            return False
        except TypeError, e:
    ...
    res = create_track_file_from_trackid(
        maindir, trackid, song, artist, mbconnect=mbconnect)
    return res
...
def create_track_files_from_artist(maindir, artist, mbconnect=None, maxsongs=100):
    ...
    for song in allsongs:
        ...
        if CREATION_CLOSED:
            return cnt_created
        # create
        created = create_track_file_from_song(
            maindir, song, artist, mbconnect=mbconnect)
        if created:
            cnt_created += 1
    # done
    return cnt_created
...
def create_step10(maindir, mbconnect=None, maxsongs=500, nfilesbuffer=0, verbose=0):
    ...
    for artist in artists:
        ...
        t2 = time.time()
        print 'found', nh5, 'h5 song files in', maindir, 'in', int(t2-t1), 'seconds (pid='+str(os.getpid())+')'
        sys.stdout.flush()
        # sanity stop
...
def create_step20(maindir, mbconnect=None, maxsongs=500, nfilesbuffer=0, verbose=0):
    ...
    if verbose > 0:
        print 'most used terms retrievend, got', len(most_used_terms)
    ...
    for termid, term in enumerate(most_used_terms):
        ...
        if CREATION_CLOSED:
            return cnt_created
        # verbose
        print 'doing term', termid, 'out of', len(most_used_terms), '(pid='+str(os.getpid())+')'
        sys.stdout.flush()
        ...
        for artist in artists:
            ...
            if verbose > 0:
                print 'doing artist:', artist.name, '(term =', term, ')'
            cnt_created += create_track_files_from_artist(maindir, artist,
                                                          mbconnect=mbconnect,
                                                          maxsongs=maxsongs)
            # sanity stop
            nh5 = count_h5_files(maindir)
            print 'found', nh5, 'h5 song files in', maindir, '(pid='+str(os.getpid())+')'
            sys.stdout.flush()
            if nh5 > TOTALNFILES - nfilesbuffer:
                return cnt_created
    # done
...
def create_step30(maindir, mbconnect=None, maxsongs=500, nfilesbuffer=0):
    ...
    for lineid, line in enumerate(lines):
        ...
        if nh5 > TOTALNFILES - nfilesbuffer:
            return cnt_created
        # verbose
        print 'doing line', lineid, 'out of', len(lines), '(pid='+str(os.getpid())+')'
        sys.stdout.flush()
        # parse line
...
def create_step40(maindir, mbconnect=None, maxsongs=100, nfilesbuffer=0):
    ...
    for argsid, args in enumerate(all_args):
        ...
        if nh5 > TOTALNFILES - nfilesbuffer:
            return cnt_created
        # verbose
        print 'doing search args', argsid, 'out of', len(all_args), '(pid='+str(os.getpid())+')'
        sys.stdout.flush()
        # songs
...
def create_step60(maindir, mbconnect=None, maxsongs=100, nfilesbuffer=0):
    ...
    while not artist_queue.empty():
        ...
        # we keep 10 at random, the radius of artists grows faster
...
def run_steps(maindir, nomb=False, nfilesbuffer=0, startstep=0, onlystep=-1, idxthread=0):
    ...
    if nomb:
        ...
        import pg
        connect = pg.connect('musicbrainz_db', 'localhost', -
                             1, None, None, MBUSER, MBPASSWD)
    ...
    try:
        ...
        if startstep <= 10 or onlystep == 10:
            cnt_created += create_step10(maindir, connect,
                                         maxsongs=100, nfilesbuffer=nfilesbuffer)
            if CREATION_CLOSED:
                startstep = onlystep = 999999
        # step 20
        ...
        if startstep <= 60 or onlystep == 60:
            cnt_created += create_step60(maindir, connect,
                                         maxsongs=50, nfilesbuffer=nfilesbuffer)
            if CREATION_CLOSED:
                startstep = onlystep = 999999
    except KeyboardInterrupt:
        close_creation()
